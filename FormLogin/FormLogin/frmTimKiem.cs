...
namespace FormLogin
{
    public partial class frmTimKiem : Form
    {
        ...
        private void btnTimkiem_Click(object sender, EventArgs e)
        {
            ...
            {
                sql = "select MaHD,FORMAT (NgayTao,'dd/MM/yyyy') as 'Ngaytao', TongTien from HoaDon where 1=1";
                if (txtMahd.Text != "")
                {
        ...
        private void dgvTimHD_DoubleClick(object sender, EventArgs e)
        {
                ...
                MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                sql = "select NgayTao from HoaDon where MaHD='" + mahd + "'";
                cmd = new SqlCommand(sql, conn);
                reader = cmd.ExecuteReader();
                if(reader.Read())
                {
                    hd.dtngayban.Value = (DateTime)reader.GetValue(0);
                    hd.dtngayban.Format = DateTimePickerFormat.Custom;
                    hd.dtngayban.CustomFormat = "dd/MM/yyyy";
                    reader.Close();
                }    
