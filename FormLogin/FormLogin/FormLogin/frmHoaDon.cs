...
namespace FormLogin
{
    public partial class frmHoaDon : Form    
    {
        ...
        private void btnadd_HD_Click(object sender, EventArgs e)
        {
            ...
            int numRows = dgvHD.Rows.Count;
            for (int i = 0; i < numRows; i++)
            {
                try
                {
                    int max = dgvHD.Rows.Count - 1;
                    dgvHD.Rows.Remove(dgvHD.Rows[max]);

                }
                catch (Exception exe)
                {
                    MessageBox.Show("Bạn không thể xóa " + exe, "WTH",
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < dgvHD.RowCount; i++)
            {
                try
                {
                    sql = "insert into CTHoaDon (MaHD, MaDia, SoLuong, ThanhTien) values('" + txtMahd.Text + "','"
                    + dgvHD.Rows[i].Cells["Mã đĩa"].Value + "','"
                    + dgvHD.Rows[i].Cells["Số lượng"].Value + "','"
                    + dgvHD.Rows[i].Cells["Thành tiền"].Value + "')";
                    cmd = new SqlCommand(sql, conn);
                    cmd.ExecuteNonQuery();
